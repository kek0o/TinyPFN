...
class Prior:
    ...
    def __init__(
        ...
        assert 2 <= min_classes <= max_classes, "Invalid class range: need 2 <= min_classes <= max_classes"
...
class SCMPrior(Prior):
    ...
    def generate_dataset(self, params: Dict[str, Any]) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        while True:
            ...
            # Use the min_classes from params for sanity check
            min_classes = params.get("min_classes", 2)
            if (d > 0).all() and self.sanity_check(X, y, params["train_size"], min_classes=min_classes):
        ...
        for gp_idx in range(num_gps):
            ...
            for subgp_idx in range(num_subgps_in_gp):
                ...
                for ds_idx in range(actual_subgp_size):
                    # MODIFIED: Sample num_classes respecting min_classes and max_classes
                    # No more 50% bias towards binary!
                    ...
                    params = {
                        ...
                        **subgp_sampled_hp,  # sampled HPs for this group
                        ...
                        "min_classes": self.min_classes,  # Pass min_classes for sanity check
                        "device": self.device,
                    }
